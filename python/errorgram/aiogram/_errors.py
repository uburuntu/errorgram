...
class BotKicked(EnrichedError, TelegramForbiddenError):
    'The bot was removed or banned from the chat.'


class BotNotMember(EnrichedError, TelegramForbiddenError):
    'The bot lacks the chat membership required for this operation.'


class ChatGroupDeleted(EnrichedError, TelegramForbiddenError):
...
class FileIdInvalid(EnrichedError, TelegramBadRequest):
    'The server could not resolve the supplied file_id.'


class FileIdMissing(EnrichedError, TelegramBadRequest):
    'The file_id argument is missing or empty.'


class FileUnavailable(EnrichedError, TelegramBadRequest):
    'The file could not be downloaded with the supplied identifier.'


class FileUrlContentTypeInvalid(EnrichedError, TelegramBadRequest):
    'The retrieved web content is unsuitable for the requested operation.'


class FileUrlFetchFailed(EnrichedError, TelegramBadRequest):
    'Telegram could not retrieve content from the HTTP URL.'


class FileUrlInvalid(EnrichedError, TelegramBadRequest):
    'The HTTP URL was rejected.'


class FileUrlUploadFailed(EnrichedError, TelegramBadRequest):
    'Telegram could not upload the file from its URL.'


class FormatEntityNotObject(EnrichedError, TelegramBadRequest):
    'An item in the message entities array is not a JSON object.'


class FormatEntityTypeUnsupported(EnrichedError, TelegramBadRequest):
    'A message entity specifies an unsupported type.'


class FormatParseModeUnsupported(EnrichedError, TelegramBadRequest):
    'The requested parse_mode is unsupported.'


class MarkupInvalidJson(EnrichedError, TelegramBadRequest):
    'The reply_markup argument is not valid JSON.'


class MarkupNotObject(EnrichedError, TelegramBadRequest):
    'The reply_markup value is not a JSON object.'


class MarkupTooLong(EnrichedError, TelegramBadRequest):
    'Telegram rejected the size of the reply markup.'


class MemberIsAdministrator(EnrichedError, TelegramBadRequest):
    'The operation was rejected because the target user is a chat administrator.'


class MessageCopyNotFound(EnrichedError, TelegramBadRequest):
...
class MessageDeleteNotFound(EnrichedError, TelegramBadRequest):
    'The message to delete could not be resolved.'


class MessageEditNotFound(EnrichedError, TelegramBadRequest):
    'The message to edit could not be resolved.'


class MessageForwardNotFound(EnrichedError, TelegramBadRequest):
    'The message to forward could not be resolved.'


class MessageIdentifierMissing(EnrichedError, TelegramBadRequest):
    'The request did not specify a message identifier for the selected operation.'


class MessageIdentifiersMissing(EnrichedError, TelegramBadRequest):
...
class MessagePinNotFound(EnrichedError, TelegramBadRequest):
    'The message to pin could not be resolved.'


class MessageReplyNotFound(EnrichedError, TelegramBadRequest):
    'The message being replied to could not be resolved.'


class PermissionsInvalidJson(EnrichedError, TelegramBadRequest):
    'The permissions argument is not valid JSON.'


class PermissionsNotObject(EnrichedError, TelegramBadRequest):
...
class TextEmpty(EnrichedError, TelegramBadRequest):
    'The message text is empty.'


class TextTooLong(EnrichedError, TelegramBadRequest):
...
class UserDeactivated(EnrichedError, TelegramForbiddenError):
