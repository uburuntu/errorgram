---
title: "Get started"
description: "Build Errorgram from source, install the Python or JavaScript package, and classify your first Telegram Bot API error."
---
...
Use `uv pip install -e '.[aiogram]'` to include aiogram. See [Python & aiogram](/python/) for concrete exceptions and adapter behavior.
...
The package includes ESM JavaScript and TypeScript declarations. The core has no runtime dependencies. See [JavaScript & grammY](/javascript/) for the optional framework adapter.
...
Pass the API method when available. Keep your existing fallback for an `unknown`, `ambiguous`, or `insufficient_context` result. Read [how matching works](/matching/) before using a classification to branch.

[Read this page as Markdown](/getting-started.md)
